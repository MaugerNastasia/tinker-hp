...
c
      ...
      TYPE BEAD_TYPE
        ...
        real*8, allocatable :: v(:,:), a(:,:)
        real*8 :: dedv
        real*8 :: epot, etot, eksum, ekin(3,3)
        real*8 :: eintra
        real*8 :: einter
        ...
        !
        integer :: nualt
        real*8, allocatable :: udalt(:,:,:),upalt(:,:,:)
     ...
     &          + eigmat_ctr(i,k)*eigmat(j,k)
            enddo
          enddo; enddo
          contractor_mat=contractor_mat*sqrt(nbeads_ctr*1.d0/nbeads)
...
c   
     ...
     &                send_pos,send_vel,send_forces)
      ...
      integer status(MPI_STATUS_SIZE),tagmpi
      real*8, allocatable :: buffer(:,:),indexposcomm(:,:,:,:)
...
c   
     ...
     $     MPI_REAL8,i,tagmpi,MPI_COMM_WORLD,reqrec(i),ierr)
        ...
        do k = 1, size(beads)
         ...
         call MPI_ISEND(buffer,nsend*beads(k)%nloc,MPI_REAL8
     $   ,0,tagmpi,MPI_COMM_WORLD,reqsend(1),ierr)
         call MPI_WAIT(reqsend(1),status,ierr)
...
c
     ...
     $     ,nsend*polymer%nloccomm(k,i+1)
     $     ,MPI_REAL8,i,tagmpi,MPI_COMM_WORLD,reqsend(i),ierr)
        ...
        do k = 1, size(beads)
          allocate(buffer(nsend,beads(k)%nloc))
          call MPI_IRECV(buffer,nsend*beads(k)%nloc,MPI_REAL8
